...
- 2026-04-16: Improved GitHub release changelog handling. The release workflow now writes explicit release bodies from the merged PR's `## Release Notes` section, with `## User-Facing Impact` or `## Summary` as fallback, instead of relying on GitHub's one-line autogenerated `What's Changed` block. The PR template and maintainer workflow docs now call out that the `## Release Notes` section is the source for future published changelogs.
