...
Windows conformance reads normalize embedded CRLF goldens to the manifest LF contract. Roadmap atomic-replacement mode preservation remains asserted on Unix; Windows skips the Unix permission-bit assertion because `os.FileMode.Perm` does not preserve `0600` there.
