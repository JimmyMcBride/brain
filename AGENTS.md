...
- 2026-07-27: Approved direction: Brain is an extensible context, memory, retrieval, and workflow platform. Planning will become an official optional module after a minimal internal module framework is reviewed.
- 2026-07-27: Planning is not implemented in Brain. Standalone Plan remains the reference implementation and compatibility source; initial local migration retains `.plan/`.
- 2026-07-27: Brain Core must remain useful without Planning. Official modules start as compiled Go packages behind formal contracts; future community modules use external processes over a versioned protocol.
- 2026-07-27: Planning migration supports `local`, `github`, and `hybrid` sources only; during standalone Plan migration, `hybrid` means split local/GitHub ownership. Brain Planning never imports or implements Linear integration; legacy Linear workspaces receive standalone Plan migration guidance. Planning may propose—but not silently write—durable Brain memory.
