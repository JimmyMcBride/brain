...
func writeKarpathyGuidelines(b *strings.Builder) {
	...
	b.WriteString("### 1. Think Before Coding\n\n")
	b.WriteString("**Don't assume. Don't hide confusion. Surface tradeoffs.**\n\n")
	b.WriteString("Before implementing:\n")
	b.WriteString("- State your assumptions explicitly. If uncertain, ask.\n")
	b.WriteString("- If multiple interpretations exist, present them; don't pick silently.\n")
	b.WriteString("- If a simpler approach exists, say so. Push back when warranted.\n")
	b.WriteString("- If something is unclear, stop. Name what's confusing. Ask.\n\n")
	b.WriteString("### 2. Simplicity First\n\n")
	b.WriteString("**Minimum code that solves the problem. Nothing speculative.**\n\n")
	b.WriteString("- No features beyond what was asked.\n")
	b.WriteString("- No abstractions for single-use code.\n")
	b.WriteString("- No flexibility or configurability that wasn't requested.\n")
	b.WriteString("- No error handling for impossible scenarios.\n")
	b.WriteString("- If you write 200 lines and it could be 50, rewrite it.\n\n")
	b.WriteString("Ask yourself: \"Would a senior engineer say this is overcomplicated?\" If yes, simplify.\n\n")
	b.WriteString("### 3. Surgical Changes\n\n")
	b.WriteString("**Touch only what you must. Clean up only your own mess.**\n\n")
	b.WriteString("When editing existing code:\n")
	b.WriteString("- Don't improve adjacent code, comments, or formatting unless the task requires it.\n")
	b.WriteString("- Don't refactor things that aren't broken.\n")
	b.WriteString("- Match existing style, even if you'd do it differently.\n")
	b.WriteString("- If you notice unrelated dead code, mention it; don't delete it.\n\n")
	b.WriteString("When your changes create orphans:\n")
	b.WriteString("- Remove imports, variables, and functions that your changes made unused.\n")
	b.WriteString("- Don't remove pre-existing dead code unless asked.\n\n")
	b.WriteString("The test: Every changed line should trace directly to the user's request.\n\n")
	b.WriteString("### 4. Goal-Driven Execution\n\n")
	b.WriteString("**Define success criteria. Loop until verified.**\n\n")
	b.WriteString("Transform tasks into verifiable goals:\n")
	b.WriteString("- \"Add validation\" -> \"Write tests for invalid inputs, then make them pass\"\n")
	b.WriteString("- \"Fix the bug\" -> \"Write a test that reproduces it, then make it pass\"\n")
	b.WriteString("- \"Refactor X\" -> \"Ensure tests pass before and after\"\n\n")
	b.WriteString("For multi-step tasks, state a brief plan:\n\n")
	b.WriteString("```text\n")
	b.WriteString("1. [Step] -> verify: [check]\n")
	b.WriteString("2. [Step] -> verify: [check]\n")
	b.WriteString("3. [Step] -> verify: [check]\n")
	b.WriteString("```\n\n")
	b.WriteString("Strong success criteria let you loop independently. Weak criteria such as \"make it work\" require constant clarification.\n")
}
